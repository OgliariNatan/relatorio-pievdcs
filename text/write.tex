...
\par Para oferecer suporte emocional e motivacional às vítimas de violência doméstica, a plataforma inclui um recurso de mensagens motivacionais. Essas mensagens são exibidas na página inicial, com o objetivo de fortalecer a autoestima e incentivar a busca por ajuda.
\begin{figure}[H]
   \center
...
\par As mensagens são obtidas através de uma API de citações motivacionais, garantindo variedade e relevância. A cada periodo estipulado à plataforma, uma nova mensagem é exibida, proporcionando um ambiente acolhedor e encorajador para as vítimas. Este recurso visa criar um espaço de apoio emocional. A plataforma de consulta as mensagens denomina-se \textbf{ZenQuotes API}, que é uma API gratuita que fornece citações motivacionais e inspiradoras de diversos autores. A integração com esta API permite que a plataforma exiba mensagens motivacionais atualizadas regularmente, contribuindo para o bem-estar emocional das vítimas de violência doméstica.
A API fornece frases no indioma inglês, e para garantir a acessibilidade e compreensão das mensagens, foi implementado um sistema de tradução automática utilizando a biblioteca \textit{Argostranslate} do Python. Este sistema traduz as citações para o português antes de exibi-las na plataforma, de forma gratuita e local. 
