...
\par A página inicial da plataforma tem por foco a concientização dos público alvo, e acesso a informações relevantes sobre a violência doméstica. Para isso, foram implementados \textit{cards informativos} dinâmicos, que exibem dados estatísticos atualizados, dicas de segurança, contatos de emergência e links para recursos de apoio.
...
\item \textbf{Segurança Avançada}: 
   \begin{itemize}
      ...
      \item Camadas extras de segurança com decoradores para controle de acesso por grupo institucional, usuários autentificados e usuários ativos (algoritmo \ref{alg:seg_extras}).
   \end{itemize}
...
\par O framework Django possui verificaçãoe de segurança robustas, no entanto implementamos camadas extras de segurança, utilizando decoradores personalizados que verificam se o usuário está autenticado, ativo e pertence a grupos institucionais permitidos antes de conceder acesso às funcionalidades sensíveis da plataforma.
...
\begin{itemize}
    ...
    \item \textbf{Módulo da segurança pública} Polícia Militar, Polícia Penal, Polícia Civil, Polícia Científica
...
\par Entre julho de 2025 e março de 2026, foram realizados mais de 30 pull requests (PRs \#74 a \#103), demonstrando um ritmo consistente de desenvolvimento. Cada PR representa um conjunto coeso de funcionalidades testadas e validadas antes da integração.
...
\par O projeto PIEVDCS encontra-se em estágio avançado de desenvolvimento, com aproximadamente \textbf{80\% das funcionalidades previstas já implementadas e testadas}. Entre os avanços recentes (março 2026), destacam-se as a implementação do sistema de mensageria em tempo real com WebSockets, a atualização do formulário de Medidas Protetivas de Urgência para tratamento técnico do período indeterminado, e a incorporação do campo de violência vicária, ampliando a capacidade de registro detalhado das ocorrências.
