...
A seção destinada à Polícia Militar é uma das mais críticas, pois fornece acesso rápido a informações sobre medidas protetivas de urgência. Originalmente, a funcionalidade permitia a consulta e visualização desses dados, mas foram identificados gargalos de performance significativos.

\par Em testes com uma massa de dados elevada, o tempo de resposta para consultas completas ultrapassava 70 segundos, tornando a ferramenta impraticável para uso em campo. Para resolver essa questão, foi realizado um trabalho de otimização focado em três frentes principais:
\begin{itemize}
    \item \textbf{Otimização de Queries:} As consultas SQL foram reestruturadas para reduzir a complexidade e o número de junções (joins) necessárias.
    \item \textbf{Indexação de Banco de Dados:} Foram criados índices estratégicos nas tabelas, permitindo que o banco de dados localize os registros de forma muito mais eficiente.
    \item \textbf{Paginação com Auto Carregamento:} Implementou-se a paginação dos resultados com carregamento automático de novas páginas conforme o usuário rola a pesquisa (\textit{infinite scroll}), tornando possível consultar grandes volumes de medidas protetivas de forma leve e contínua.
\end{itemize}

\par Como resultado, o tempo de execução das consultas foi reduzido para uma média de 0.2 segundos, um ganho de performance superior a 99\%, garantindo que os agentes de segurança tenham acesso quase instantâneo às informações (Figura \ref{fig:consulta_pm_otimizada}).

\begin{itemize}
   \item \textbf{Página inicial da Polícia Militar:} Consulta de  \textit{Vitimas} ou \textit{Agressores}, por \textit{Nome} ou \textit{CPF}; Consulta de todas as Medidas Protetivas e \textit{Cards informativos}
\end{itemize}
...
\par Testes realizados em janeiro de 2026 com a base de dados de teste contendo mais de 500 registros fictícios demonstraram melhorias significativas:
...
\hline
Carregamento inicial (PM) & 8.5s & 1.2s & 85.8\% \\
\hline
Consulta de vítimas & 5.3s & 0.9s & 83.0\% \\
\hline
...
\par A principal otimização consistiu na implementação de \textbf{paginação dinâmica via HTMX} para o carregamento de listas extensas das medidas protetivas. Anteriormente, todos os registros eram carregados simultaneamente, causando lentidão proporcional ao volume de dados. Com a nova abordagem, os registros são carregados progressivamente, reduzindo o tempo de resposta inicial e o consumo de recursos.
...
\begin{itemize}
    ...
    \item \textbf{Consulta otimizada de banco de dados}: Implementação de queries específicas para busca total de vítimas com índices otimizados no PostgreSQL.
\end{itemize}

\par Essas otimizações garantem que a consulta ao histórico completo de vítimas e agressores pela Polícia Militar seja realizada de forma eficiente, permitindo análise adequada das informações após o atendimento inicial e estabilização da ocorrência.
...
\hline
Total de Medidas Protetivas cadastradas & 6.000 \\
\hline
...
\par A figura \ref{fig:consulta_pm_sem_com_otimizada} ilustra a consulta do módulo da Polícia Militar, onde é possível visualizar a implementação sem otimização e da paginação dinâmica que permite navegação eficiente mesmo com a base completa de 6.000 registros de teste. A interface apresenta tempos de resposta consistentes independente do volume total de dados cadastrados.
