...
\begin{itemize}
    ...
    \item \textbf{TRL 3 e 4 (Concluído 3, em andamento 4(80\%) - Pesquisa Aplicada):} Os conceitos tecnológicos foram estabelecidos e validados em ambiente de desenvolvimento. A arquitetura Django com PostgreSQL, sistema de autenticação customizado baseado em instituições e integração entre módulos foram testados com sucesso em laboratório (ambiente de desenvolvimento local). Validação funcional dos componentes críticos: sistema de cadastro de vítimas/agressores, formulários de medidas protetivas, sistema de notificações em tempo real via WebSockets e dashboards estatísticos.
...
\par O módulo de mensageria implementa comunicação em tempo real entre usuários e instituições da plataforma utilizando \textbf{WebSockets} via \textbf{Django Channels} e \textbf{Redis} como \textit{broker} de mensagens. Essa funcionalidade é fundamental para a coordenação ágil das ações institucionais no enfrentamento à violência doméstica.

\par A arquitetura do módulo permite o envio de notificações e mensagens de forma instantânea para usuários individuais ou para grupos institucionais inteiros (por exemplo, todas as delegacias de uma comarca), com classificação por nível de prioridade e rastreamento de status de leitura.

\par Aspectos técnicos da implementação:
\begin{itemize}
   \item \textbf{Protocolo WebSocket}: Conexão bidirecional e persistente entre cliente e servidor, permitindo troca de dados em tempo real sem necessidade de \textit{polling}\footnote{Técnica em que o cliente consulta repetidamente o servidor para verificar se há novas mensagens.};
   \item \textbf{Django Channels}: Extensão do Django que adiciona suporte a protocolos assíncronos, gerenciando as conexões WebSocket e o roteamento de mensagens;
   \item \textbf{Redis como \textit{broker}}: Armazenamento em memória de alta performance utilizado para distribuir mensagens entre múltiplos processos do servidor, garantindo escalabilidade;
   \item \textbf{Capacidade}: Sistema configurado para suportar até 1.500 conexões simultâneas, com tempo de expiração de mensagens de 10 segundos;
   \item \textbf{Prioridades}: Classificação em três níveis — Normal, Urgente e Crítica — permitindo que situações de risco imediato sejam destacadas;
   \item \textbf{Status de leitura}: Rastreamento do estado de cada mensagem (Não lida, Lida, Arquivada), garantindo auditabilidade das comunicações institucionais.
\end{itemize}
...
\par Testes realizados em janeiro de 2026 com a base de dados de teste contendo 6.000 registros fictícios de Medidas Protetivas de Urgência demonstraram melhorias significativas:
...
\par O Pull Request \#103, mesclado em 30 de janeiro de 2026, consolidou 17 arquivos modificados com 1.989 adições e 555 deleções de código, representando uma refatoração significativa focada em performance e usabilidade.
...
\par Entre julho de 2025 e janeiro de 2026, foram realizados mais de 30 pull requests (PRs \#74 a \#103), demonstrando um ritmo consistente de desenvolvimento. Cada PR representa um conjunto coeso de funcionalidades testadas e validadas antes da integração.
...
\par O projeto PIEVDCS encontra-se em estágio avançado de desenvolvimento, com aproximadamente \textbf{40\% das funcionalidades previstas já implementadas e testadas}. Entre os avanços recentes (janeiro 2026), destacam-se as otimizações de performance no módulo da Polícia Militar, com implementação de paginação dinâmica via HTMX e melhorias nas consultas ao banco de dados PostgreSQL.
